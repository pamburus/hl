...
fn test_same_size_replacement_detected() {
    ...
    let handle = thread::spawn(move || {
        thread::sleep(Duration::from_millis(50));
        // Atomically replace via rename so the new file gets a fresh inode,
        // which is detectable via identity comparison during recheck_cadence.
        // (In-place O_TRUNC keeps the same inode; on some platforms kqueue may
        // not deliver an event for the transient size=0 state before the write
        // fills the file back to the same size, making detection unreliable.)
        let tmp = dir2.join("samesize.tmp");
        create_file(&tmp, b"bbbb\n");
        fs::rename(&tmp, &path2).unwrap();
    });

    // We should get the new content within recheck_cadence.
    let data = read_n_bytes(&mut reader, 5, Duration::from_secs(5));
    handle.join().unwrap();

    let s = String::from_utf8_lossy(&data);
    assert!(s.contains("bbbb"), "must read replacement content, got: {s:?}");
}
