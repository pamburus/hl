...
///
/// On macOS: custom kqueue backend.
/// On Windows: hybrid ReadDirectoryChangesW + adaptive GetFileInformationByHandle
///   polling (see backend/windows.rs for rationale).
/// Elsewhere: notify's RecommendedWatcher.
#[cfg(target_os = "macos")]
...
#[cfg(windows)]
pub fn new_native(tx: SyncSender<WakeSignal>, _poll_interval: Duration) -> Result<Box<dyn Backend>, Error> {
    windows::WindowsNativeBackend::new(tx).map(|b| Box::new(b) as Box<dyn Backend>)
}
...
/// Constructs the adaptive polling backend used for unreliable paths (network
/// shares, FUSE mounts, unrecognised filesystems). Uses IIR-based adaptive
/// interval estimation rather than a fixed poll interval.
