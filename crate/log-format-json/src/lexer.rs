...
pub type InnerLexer<'s> = logos::Lexer<'s, Token<'s>>;
...
mod tests {
    ...
    fn test_simple_object() {
        ...
        assert_eq!(next!(lexer), Scalar(Bool(true)));
        assert_eq!(next!(lexer), CompositeEnd);
        assert_eq!(next!(lexer), CompositeBegin(Field(Plain(b"d"))));
        assert_eq!(next!(lexer), CompositeBegin(Array));
        assert_eq!(next!(lexer), Scalar(String(Plain(b"e"))));
        assert_eq!(next!(lexer), Scalar(Number(b"42")));
