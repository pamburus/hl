...
mod tests {
    ...
    fn test_simple_object() {
        let input = br#"{"a":{"b":true,"d":["e",42,null]}}"#.into();
        let mut lexer = Lexer::from_source(&input);
        assert_eq!(next!(lexer), EntryBegin);
        assert_eq!(next!(lexer), CompositeBegin(Field(Plain("a".into()))));
        assert_eq!(next!(lexer), CompositeBegin(Object));
        ...
        assert_eq!(next!(lexer), Scalar(Bool(true)));
        assert_eq!(next!(lexer), CompositeEnd);
        assert_eq!(next!(lexer), CompositeBegin(Field(Plain("d".into()))));
        assert_eq!(next!(lexer), CompositeBegin(Array));
        assert_eq!(next!(lexer), Scalar(String(Plain("e".into()))));
        assert_eq!(next!(lexer), Scalar(Number("42".into())));
