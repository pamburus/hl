...
- **Partial trailing entry**: A trailing fragment without its delimiter is not emitted as a complete entry until its delimiter arrives.
...
- **SC-003**: On a filesystem where native notifications deliver no events (network/unreliable), appended entries still appear within one polling interval (default 1 second) in 100% of trials.
