...
- [X] T045 [P] [US6] hl follow integration test exercising the new crate path (append + rotation visible through `hl -F`) in an in-crate `#[cfg(test)]` module under `src/app.rs`
...
- [X] T053 Run `just uncovered` and add tests for changed lines lacking coverage; document any environment-gated exclusions (SC-009, constitution V).
  - **Coverage exclusion fixed:** added `/tests/[^/]*\.rs$` to the ignore list in `build/ci/coverage.sh` so the crate's `tests/` integration files are excluded as test code (they were previously counted); relocated the inline `IntervalEstimator` test module into `watch/interval/tests.rs` so it matches the existing `/tests\.rs$` exclusion convention. No test code is measured now.
  - **Tests added:** unit tests for the adaptive `IntervalEstimator`; integration tests (`crates/fsmon/tests/coverage_paths.rs`) covering the core `Watcher` API (`recv`/`engine`, data + `Removed` events), multi-source `next_chunk`, in-place truncation re-read, the rotation drain path, and `classify()` NUL-path rejection.
  - **Environment-gated / not deterministically reachable on the dev box (deferred to T056 / documented):** `watch/backend/native.rs` (the `notify` wrapper; inactive on macOS, which uses `kqueue_macos`); `watch/backend/poll.rs` and the polling branch of `engine.rs` (only selected for `NotConfirmed` filesystems — needs a remote/FUSE/unknown mount); `classify/macos.rs:39` (non-known-local fs); and defensive error branches (`File::open` failing mid-rotation, `Interrupted`/`Disconnected`) that require fault injection.
  - **Finding (separate from coverage):** `engine::degrade_to_polling` (FR-014 / US5 runtime degradation) is `#[allow(dead_code)]` with **no callers and no test**, despite T041–T043 being marked complete. The native→polling migration is defined but not wired into the live loop — needs follow-up.
