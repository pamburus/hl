...
│   ├── config/          # Submodule directory
│   │   └── mod.rs       # PagerConfig, PagerProfile, PagerRole, ConditionalArgs structs
...
1. Create `src/pager/config/mod.rs`:
   - `PagerConfig` struct with `candidates: Vec<PagerCandidate>` and `profiles: Vec<PagerProfile>`
   - `PagerCandidate` struct (`kind: PagerCandidateKind`, `if: Option<Condition>`)
   - `PagerCandidateKind` enum (env reference or profile name string)
   - `PagerProfile` struct (`name`, `command`, `args`, `env`, `delimiter`, `modes`, `conditions`)
   - `PagerModes` struct wrapping `view: PagerRoleConfig` and `follow: PagerRoleConfig`
   - `PagerRoleConfig` struct (enabled, args)
   - `ConditionalArgs` struct (`if: Condition`, `args`, `env`)
...
1. Create `src/pager/selection.rs`:
   ...
   - `select_for_view()` method implementing FR-029 precedence
   - `select_for_follow()` method implementing FR-029a precedence
...
1. Modify `src/output.rs`:
   - `Pager::new()` → `Pager::from_selection(SelectedPager)`
   - Accept `SelectedPager` containing the fully resolved command, env, and delimiter
