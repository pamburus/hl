...
function test() {
    ...
    cargo build
    ${MAIN_EXECUTABLE:?} --no-config > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config --help > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config --config=etc/defaults/config-k8s.yaml > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config --config=etc/defaults/config-ecs.yaml > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config --shell-completions bash > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config --man-page > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config --list-themes > /dev/null
    ${MAIN_EXECUTABLE:?} --no-config sample/prometheus.log -P > /dev/null
    echo "" | ${MAIN_EXECUTABLE:?} --no-config --concurrency 4 > /dev/null
} 
