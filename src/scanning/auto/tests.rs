...
//
// These tests cover all cases and edge cases to prevent regressions in the AutoDelimitSearcher
// logic, which was previously buggy. The AutoDelimitSearcher is responsible for finding newline
// delimiters while skipping continuation lines (lines starting with '}', ' ', or '\t').
//
...
//
// The bug that was fixed: The AutoDelimitSearcher's partial_match_r and edge mode logic
// had issues correctly identifying when a newline at the end of a buffer might be followed
// by a continuation character in the next chunk, leading to incorrect block splitting in
// the indexer.
...
// Regression test: ensure the bug that was fixed stays fixed
#[rstest]
#[case(b"line1\n", b"line2\n", b"line3\n")]
#[case(b"a\n", b"b\n", b"c\n")]
