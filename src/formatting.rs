                ...
                if !self.cfg.hide_empty_fields || !v.is_empty() {
                    let result = fs.transact(s, |fs, s| {
                        match self.format_field(s, k, *v, fs, Some(&self.cfg.fields)) {
                            ...
                            FieldFormatResult::ExpansionNeeded(analysis) => Err(analysis),
                        }
                    });
...
impl<'a> FieldFormatter<'a> {
    ...
    fn format<S: StylingPush<Buf>>(
        ...
        match result {
            ...
            ValueFormatResult::ExpansionNeeded(analysis) => FieldFormatResult::ExpansionNeeded(analysis),
        }
    }
    ...
    fn format_value<S: StylingPush<Buf>>(
        ...
        match value {
            RawValue::String(value) => {
                ...
                if result.aborted {
                    return ValueFormatResult::ExpansionNeeded(result.analysis);
...
enum ValueFormatResult {
    Ok,
    ExpansionNeeded(Option<Analysis>),
}
