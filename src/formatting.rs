            ...
            RawValue::String(value) => {
                let result = s.element(Element::String, |s| {
                    s.batch(|buf| {
                        ...
                        ValueFormatAuto.format(value, buf, xsa).unwrap()
                    })
                });
                match result {
...
pub mod string {
    ...
    impl Format for ValueFormatAuto {
        ...
        fn format<'a>(
            ...
            _ = buf.with_auto_trim(
                |buf| ValueFormatRaw.format(input, buf, xsa),
            ...
            if !has_control && has_non_whitespace {
                if !has_backtick && (!has_extended_space || matches!(xsa, ExtendedSpaceAction::Inline)) {
                    buf.push(b'`');
                    buf.push(b'`');
                    buf[begin..].rotate_right(1);
                    return Ok(FormatResult::Ok(None));
    ...
    impl Format for MessageFormatAutoQuoted {
        ...
        fn format<'a>(
            ...
            let begin = buf.len();
            _ = buf.with_auto_trim(|buf| MessageFormatRaw.format(input, buf, xsa), AutoTrimFlags::empty())?;
    ...
    impl Format for MessageFormatDelimited {
        ...
        fn format<'a>(
            ...
            const XS: Mask = mask!(Flag::NewLine);
            if mask.intersects(XS) && matches!(xsa, ExtendedSpaceAction::Abort) {
                buf.truncate(begin);
                return Ok(FormatResult::Aborted);
            }
