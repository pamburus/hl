...
mod tests {
    ...
    impl<'r, 'a, T> FormatToVec<model::RecordWithSource<'r, 'a>> for T
    where
        T: RecordWithSourceFormatter,
    {
        fn format_to_vec(&self, rec: model::RecordWithSource<'r, 'a>) -> Vec<u8> {
            let mut buf = Vec::new();
            (&self).format_record(&mut buf, 0..0, rec);
            buf
        }
    ...
    impl<'r, 'a, T> FormatToString<model::RecordWithSource<'r, 'a>> for T
    where
        T: RecordWithSourceFormatter,
    {
        fn format_to_string(&self, rec: model::RecordWithSource<'r, 'a>) -> String {
            String::from_utf8(self.format_to_vec(rec)).unwrap()
        }
    }
