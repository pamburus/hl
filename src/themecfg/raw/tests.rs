...
#[test]
fn test_v0_theme_merge_with_overlapping_level_overrides() {
    let base = raw_theme("v0-base-with-level");
    let patch = raw_theme("v0-patch-with-level");
    ...
    assert_eq!(message.unwrap().foreground, Some(Color::RGB(RGB(0, 0, 255))));
}
