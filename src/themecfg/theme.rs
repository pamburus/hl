...
impl Theme {
    ...
    pub fn load_with_overlays(dirs: &AppDirs, name: &str, overlays: &[impl AsRef<str>]) -> Result<Self> {
        ...
        for name in overlays {
            ...
            log::debug!("merging overlay '{}' into theme chain", name);
            theme = theme.merged(overlay);
        }

        log::trace!("resolving final theme");
        theme.resolve()
    }
    ...
    pub fn load_raw(dirs: &AppDirs, name: &str) -> Result<RawTheme> {
        let themes_dir = Self::themes_dir(dirs);
        log::trace!(
            "searching for theme '{}' in custom directory: {}",
            name,
            themes_dir.display()
        );

        let theme = match Self::load_from(&themes_dir, name) {
            Ok(v) => {
                log::debug!("loaded theme '{}' from custom directory", name);
                Ok(v)
            }
            Err(Error::ThemeNotFound { .. }) => {
                ...
                match Self::load_embedded::<Assets>(name) {
                    ...
                    Err(Error::ThemeNotFound { name, mut suggestions }) => {
                        ...
                        log::trace!("theme '{}' not found in embedded resources", name);
    ...
    pub(super) fn load_embedded<S: RustEmbed>(name: &str) -> Result<RawTheme> {
        for format in Format::iter() {
            ...
            if let Some(file) = S::get(&filename) {
                log::trace!("found embedded theme file: {}", filename);
                let inner =
    ...
    pub(super) fn load_from(dir: &Path, name: &str) -> Result<RawTheme> {
        ...
        for format in Format::iter() {
            ...
            match std::fs::read(&path) {
                Ok(data) => {
                    log::trace!("found custom theme file: {}", path.display());
