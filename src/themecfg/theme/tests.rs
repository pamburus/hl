...
#[test]
fn test_unsupported_file_type_with_path() {
    let result = Theme::load_from(&PathBuf::from("."), "path/to/theme.txt");

    assert!(result.is_err(), "Should fail with unsupported file extension");

    match result {
        Err(Error::UnsupportedFileType { path, extension }) => {
            assert_eq!(path.as_ref(), "path/to/theme.txt");
...
#[test]
fn test_overlay_parse_error() {
    let result = Theme::load_with_overlays(&dirs(), "test", &["malformed"]);

    assert!(result.is_err(), "Should fail when overlay has parse error");

    match result {
        Err(Error::FailedToLoadCustomTheme { .. }) | Err(Error::FailedToLoadEmbeddedTheme { .. }) => {}
        Err(e) => panic!("Expected parse error for malformed overlay theme, got: {:?}", e),
